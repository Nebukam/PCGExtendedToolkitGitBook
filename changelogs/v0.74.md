...
I now build a [Doubly Connected Edge List](https://en.wikipedia.org/wiki/Doubly_connected_edge_list) from the clusters, which is infinitely more reliable than my previous angle-based approach.

The primary reason is that as I added Manhattan and Chebyshev voronoi modes, the topology of the cluster became slightly more complex and "rigid" with perfect orthogonal angles that created a LOT of false positive/negative in the existing cell detection code, and it was a mess.

I lost of a lot of time trying to fix it without proper success so I ended up taking another more reliable approach entirely.

</details>

This is an important one as <mark style="color:$warning;">it will break some of your existing settings</mark>; **specifically node count constraints** : the previous code was not counting the cell count properly (adding one to all counts), this is not true anymore. That means cell constraints with the old value will not return the same results now.
...
* Seeds will only capture the cells that _contains_ their position, instead of "close enough".\
...
Long overdue, I've been doing a lot of cleanup
