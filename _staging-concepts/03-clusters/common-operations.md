...
| Connect each point to its K nearest | **Build Custom Graph** | Probe-based — configure exactly which connections form. |
...
See [Cluster-Path Interop](cluster-path-interop.md) for the full back-and-forth workflow.
...
- [Building Clusters](building-clusters.md) — Generation strategies in depth
- [Refining Clusters](refining-clusters.md) — Refinement concepts
- [Cluster Nodes](/node-library/clusters/) — Complete node reference
