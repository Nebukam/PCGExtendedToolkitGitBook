...
What PCGEx adds is a **modular configuration system** that separates *what to do* from *how to configure it*.
...
<!-- IMAGE: A filter provider node with its output connected to three different consumer nodes -->
...
<!-- IMAGE: Examples of different sub-node types showing their naming convention and visual style -->

Sub-nodes are lightweight. They don't process data themselves; they describe *how* something should be done. The consumer that receives them handles the actual processing.
...
**Rules & Selection**: Sorting Rules, Partition Rules, Match Rules, Index Pickers.
...
<!-- IMAGE: Three filter providers connected to one consumer's Filters pin -->
...
<!-- IMAGE: PCG graph showing a PCGEx cluster output flowing into a vanilla PCG Transform Points node -->
...
- [Provider/Consumer Pattern](provider-consumer.md) - Deep dive into sub-nodes and factories

## Related

**Concepts:**
- [Filters](/concepts/04-filters/) - The most common sub-node type
- [Clusters](/concepts/03-clusters/) - Where sub-nodes configure topology operations
- [Pathfinding](/concepts/06-pathfinding/) - Where heuristics sub-nodes configure traversal

**Node Library:**
- [Filter Nodes](/node-library/filters/) - All filter sub-nodes
