...
<!-- IMAGE: Hub-and-spoke diagram showing one filter provider node with its output connected to three different consumer nodes -->
...
<!-- IMAGE: Graph showing filters defined in one area, connected to named redirector, then gathered at multiple consumer locations -->
...
{% hint style="info" %}
The Filter Group's Mode setting (AND/OR) is `PCG_Overridable` and can be exposed as a graph parameter, letting users choose how additional filters combine with built-in logic.
{% endhint %}

### Parameterization

Properties marked `PCG_Overridable` in filter settings can be promoted to subgraph inputs:
...
- [Filter Overview](README.md) - Filter fundamentals
- [Composition](composition.md) - AND/OR logic, nesting groups
