...
<!-- IMAGE: Flow diagram: Collection → Staging Node → Staged Points → Handler → Scene -->
...
| Collection Type | Contains | Spawner |
|----------------|----------|---------|
| **Mesh Collection** | Static meshes | Vanilla Spawn Static Mesh + Mesh Selector Staged |
| **Actor Collection** | Actor classes (blueprints) | Vanilla Spawn Actor |
| **PCG Data Collection** | PCG Data Assets | PCGEx Load PCG Data Asset |
...
**Meshes** → Vanilla **Spawn Static Mesh** with Mesh Selector Staged (collection map) or Mesh Selector by Attribute (attribute mode)
**Actors** → Vanilla **Spawn Actor** node
**PCG Data Assets** → PCGEx's **Load PCG Data Asset** node (the one exception — no vanilla equivalent exists)
...
- [Collections](collections.md) - Collection types and organization
- [Distribution](distribution.md) - Selection strategies and categories
- [Fitting](fitting.md) - Scale, justification, and transform adjustment

## Related

**Concepts:**
- [Valency](/concepts/08-valency/) - Constraint solving using staged collections

**Node Library:**
- [Staging Nodes](/node-library/staging/) - Staging operation reference
- [Collection Types](/node-library/staging/collections/) - Collection data asset reference
