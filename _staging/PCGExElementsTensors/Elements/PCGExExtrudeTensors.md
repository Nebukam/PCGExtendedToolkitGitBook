---
icon: route
description: 'Path : Extrude Tensors - Extrude input points into paths along tensors.'
---
...
- **Child Extrusions**: When an extrusion is stopped by filters but later exits the stop region, a new "child" path can optionally continue.
...
{% hint style="warning" %}
### Stop Filters Limitation
Only spatial filters that do not rely on point attributes will work as stop conditions. Points generated during extrusion exist only in world space and do not have access to the same attributes as your input data. Use filters based on position, bounds, or other spatial criteria (e.g., Bounds Filter, Inclusion Filter, Distance Filter).
{% endhint %}
...
📋 *Visible when Transform Rotation is enabled*
...
📋 *Visible when Transform Rotation is enabled and Rotation = Align*
...
📋 *Visible when Use Per Point Max Iterations is enabled*
...
📋 *Visible when Use Max Length is enabled*
...
📋 *Visible when Use Max Points Count is enabled*
...
📋 *Visible when Do External Path Intersections is enabled*
...
📋 *Visible when Do Self Path Intersections is enabled*
...
| **Adaptive RK** | Dynamically adjusts step size based on field curvature. Takes smaller steps in curved regions, larger steps in smooth regions. Best overall quality. *Recommended for most use cases.* |
...
→ See [Path Processor Settings](../../PCGExFoundations/Core/PCGExPathProcessor.md) for common path processing options.
...
![Static Badge](https://img.shields.io/badge/Source-PCGExElementsTensors-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExElementsTensors/Public/Elements/PCGExExtrudeTensors.h)
...
Inputs: In (Point Data), Tensors (Tensor Factories), Stop Filters (Filter Factories), Paths (Point Data)
Outputs: Out (Point Data)
Nested Types: Multiple enums and structs documented inline
-->
