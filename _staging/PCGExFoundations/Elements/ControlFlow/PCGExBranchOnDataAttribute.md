---
icon: code-branch
description: 'Branch on Data - Routes data to different output pins based on a @Data domain attribute value.'
---
...
**UserDefined**: Full control over branch logic with numeric/string comparisons
**EnumInteger**: Matches attribute value against enum integer values (0, 1, 2, etc.)
**EnumName**: Matches attribute string against enum names ("Forest", "Desert", etc.)
...
<details>
<summary><strong>Branches</strong> <code>TArray<FPCGExBranchOnDataPin></code></summary>
...
📋 *Visible when Selection Mode = UserDefined*
...
📋 *Visible when Check = Numeric and Comparison = Nearly Equal or Nearly Not Equal*
...
- `"Prop_"`: Match strings starting with "Prop_"
...
📋 *Visible when Selection Mode != UserDefined and Enum Source = Picker*
...
📋 *Visible when Selection Mode != UserDefined and Enum Source = Selector*
...
![Static Badge](https://img.shields.io/badge/Source-PCGExFoundations-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/ControlFlow/PCGExBranchOnDataAttribute.h)

<!-- SOURCE: https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/ControlFlow/PCGExBranchOnDataAttribute.h -->
...
Branch Struct Properties: 7 documented (Label, Check, NumericCompare/StringCompare, NumericValue/StringValue, Tolerance)
Inherited Properties: Base class UPCGExPointsProcessorSettings (not detailed - control flow specific)
Selection Modes: UserDefined, EnumInteger, EnumName
Comparison Types: Numeric (8 operators), String (13 operators)
Inputs: Any data type
Outputs: Dynamic pins based on branches/enum values
Special Features: Dynamic pin generation, @Data domain routing, enum-based switching
Control Flow: Evaluates branches in order, first match wins, fallback to default pin
-->
