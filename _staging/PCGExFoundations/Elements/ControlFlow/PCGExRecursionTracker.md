---
icon: rotate
description: 'Break - Tracks recursion depth and conditions to control recursive subgraph execution.'
---

# Break

Tracks recursion depth and conditions to control recursive subgraph execution.
...
**Simple**: All data flows through one pin; check the tracker's "Continue" attribute to determine state
**Branch**: Data is routed to Continue or Stop pins based on recursion state
...
**Create**: Initializes a fresh tracker with MaxCount iterations remaining
**Update**: Expects existing tracker input, updates counter and tests conditions
**CreateOrUpdate**: Convenience mode that auto-creates on first run, then updates
...
📋 *Visible when Mode != Create and Output Progress = true*
...
📋 *Visible when Mode != Create and Type = Simple*
...
📋 *Visible when Mode = CreateOrUpdate*
...
📦 **Module**: `PCGExFoundations` · 📄 [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/ControlFlow/PCGExRecursionTracker.h)
...
Inherited Properties: UPCGExSettings base class (BulkInitData, CacheData, etc. - not detailed)
Tracker Types: Simple (single output), Branch (Continue/Stop outputs)
Modes: Create, Update, CreateOrUpdate
Inputs: In (any), Tracker (optional), Tracker Filters (optional), Test Data (optional)
Outputs: Continue/Stop or Out (based on type), Tracker, Progress (optional), Index (optional), Remainder (optional)
Special Features: Recursion depth tracking, iteration counting, condition testing, progress calculation
Use Cases: Recursive subgraphs, iteration control, depth limiting, condition-based breaks
-->
