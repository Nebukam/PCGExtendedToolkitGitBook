---
icon: code-branch
description: 'Uber Branch - Routes point collections to multiple output pins based on filter conditions.'
---
...
*Filter input pins are dynamically created based on Num Branches*
...
*Output pins are dynamically created based on Num Branches*

---

![Static Badge](https://img.shields.io/badge/Source-PCGExFoundations-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/ControlFlow/PCGExUberBranch.h)

<!-- SOURCE: https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/ControlFlow/PCGExUberBranch.h -->

<!-- VERIFICATION REPORT
Node-Specific Properties: 2 documented (NumBranches, AsyncChunkSize)
Internal Properties: InputLabels, OutputLabels (pin naming, not user-configurable)
Inherited Properties: UPCGExPointsProcessorSettings base class
Inputs: In (point data), dynamic filter pins (→ 0, → 1, etc.)
Outputs: Dynamic data outputs (0 →, 1 →, etc.), Default
Special Features: First-match routing, multiple filter branches, parallel batch processing
Routing Logic: Sequential filter testing, first match wins, default fallback
Performance: Async chunk processing for parallel efficiency
-->
