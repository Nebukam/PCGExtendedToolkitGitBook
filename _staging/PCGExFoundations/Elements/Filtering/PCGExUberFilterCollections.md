---
icon: layer-group
description: 'Uber Filter (Data) - Filters entire point collections based on how many points pass filter conditions.'
---
...
**All**: Strictest - requires 100% pass rate
**Any**: Most lenient - requires at least 1 passing point
**Partial**: Configurable threshold for pass rate
...
**Relative**: Threshold is a fraction (0.5 = 50%)
**Discrete**: Threshold is a number of points (10 = ten points)

📋 *Visible when Mode = Partial*
...
📋 *Visible when Mode = Partial and Measure = Relative*
...
📋 *Visible when Mode = Partial and Measure = Discrete*
...
![Static Badge](https://img.shields.io/badge/Module-PCGExFoundations-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/Filtering/PCGExUberFilterCollections.h)
...
Modes: All (100%), Any (at least 1), Partial (threshold-based)
Measures: Relative (percentage), Discrete (count)
Comparisons: 8 operators (==, !=, >=, <=, >, <, ~=, !~=)
Inputs: Point data collections, filter factories
Outputs: Inside (passing), Outside (failing)
Filter Scope: Collection-level (not point-level)
Use Cases: Quality control, presence detection, majority voting, minimum count enforcement
-->
