---
icon: clone
description: 'Discard Same - Removes duplicate point collections based on configurable similarity tests.'
---
...
**FIFO**: Preserves the first encountered collection
**LIFO**: Preserves the most recent collection
**All**: Removes all duplicates, keeping only truly unique collections
...
**AND**: Strict - requires all tests to agree
**OR**: Lenient - any matching test is sufficient
...
📋 *Visible when Test Point Count = true*
...
📋 *Visible when Test Positions = true*
...
**None**: No attribute testing
**Single**: Simple, one attribute comparison
**List**: Multiple attributes tested simultaneously
...
<details>
<summary><strong>Attribute Hash Configs</strong> <code>TArray<FPCGExAttributeHashConfig></code></summary>
...
📋 *Visible when Test Attributes Hash = List*
...
📋 *Visible when Test Attributes Hash = Single, or when Test Attributes Hash = List and Append Overridable = true*
...
**Uniques**: Hash only unique values (ignores duplicates and count)
**Count**: Hash unique values with their occurrence counts
...
![Static Badge](https://img.shields.io/badge/Source-PCGExFoundations-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/Filtering/PCGExDiscardSame.h)
...
Attribute Hash Config Properties: 4 documented (SourceAttribute, Scope, bSortInputValues, Sorting)
Modes: FIFO, LIFO, All
Test Combination: AND, OR
Comparison Types: Bounds, point count, positions, attribute hashes
Inputs: Point data collections
Outputs: Unique collections, discarded duplicates
Special Features: Tolerance-based comparisons, attribute hashing, multiple test modes
Use Cases: Deduplication, removing redundant data, unique filtering
-->
