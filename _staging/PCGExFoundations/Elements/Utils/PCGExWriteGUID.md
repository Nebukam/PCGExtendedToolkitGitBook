---
icon: fingerprint
description: 'Write GUID - Writes unique GUIDs as attributes to all points in a collection.'
---
...
**Integer**: Best for numeric operations, comparisons, hashing
**String**: Best for logging, display, debugging
...
**Constant**: Same base key for all points (uniformity)
**Attribute**: Per-point key from attribute (per-point variation)
...
📋 *Visible when Unique Key Input = Constant*
...
📋 *Visible when Unique Key Input = Attribute*
...
![Static Badge](https://img.shields.io/badge/Source-PCGExFoundations-473F69)  ·  [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/Utils/PCGExWriteGUID.h)

<!-- SOURCE: https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFoundations/Public/Elements/Utils/PCGExWriteGUID.h -->

<!-- VERIFICATION REPORT
Node-Specific Properties: 1 documented (Config), Config expands to 10 nested properties
GUID Config Properties: 10 documented (OutputAttributeName, OutputType, Format, Uniqueness, UniqueKeyInput, UniqueKeyConstant/Attribute, GridHashCollision, PositionHashCollision, PositionHashOffset, bAllowInterpolation)
Output Types: Integer (int64), String (formatted)
String Formats: 10 formats (Digits, DigitsLower, RFC 4122, braces, parentheses, hex, unique object, Base64, Base36)
Uniqueness Flags: Index, Position, Seed, Grid (bitmask)
Unique Key Input: Constant (fixed value), Attribute (per-point)
Hash Collision: Grid and Position collision thresholds to prevent FP precision issues
Determinism: GUIDs are reproducible when uniqueness factors remain constant
Inputs: Point data
Outputs: Point data with GUID attributes
Use Cases: Point identification, lookup keys, deterministic seeding, tracking, reference IDs
-->
