---
icon: filter
description: 'Filter Definition - Base class for all filter factory provider nodes.'
---
...
📋 *Visible only for filters that use external data inputs*
...
Standard filters that evaluate individual points based on their attributes and properties.

| Data Type | Description |
|-----------|-------------|
| **PCGEx \| Filter (Point)** | Filters that test point data attributes and properties |
...
📦 **Module**: `PCGExFilters` · 📄 [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExFilters/Public/Core/PCGExFilterFactoryProvider.h)

<!-- VERIFICATION REPORT
Node-Specific Properties:
- Priority (int32, default 0, PCG_Overridable)
- InitializationFailurePolicy (EPCGExFilterNoDataFallback, default Error)
- MissingDataPolicy (EPCGExFilterNoDataFallback, default Fail, conditional visibility)
Inherited Properties: Standard factory provider settings
Classes Documented:
- UPCGExFilterProviderSettings (abstract base for all filters)
- UPCGExFilterCollectionProviderSettings (base for filter collections)
Inputs: N/A (abstract base)
Outputs: Filter pin (typed by derived class)
-->
