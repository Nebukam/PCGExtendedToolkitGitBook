---
icon: puzzle-piece
description: 'Decompose : Max Boxes (Extended) - Advanced box extraction with weights, axis bias, and heuristic merge gating'
---

# Decompose : Max Boxes (Extended)

Advanced box extraction with per-node weights, axis bias, and heuristic merge gating.

## Overview

Extends the Max Boxes algorithm with additional scoring criteria. Per-node weights influence which regions are extracted first. Axis bias controls preferred box shapes (flat, tall, or cubic). Volume preference nudges extraction toward a target size range. Optional heuristic merge gating prevents merging cells across high-score cluster edges, preserving topological boundaries.

## How It Works

1. **Voxelization**: Same as Max Boxes. The cluster is mapped onto a 3D occupancy grid.
...
📋 *Visible when Transform Space = Custom*
...
📋 *Visible when Voxel Size Mode = Manual*
...
📋 *Visible when Weight is attribute-driven*
...
📋 *Visible when Weight Mode = Priority*
...
📋 *Visible when Use Heuristic Merge Gating = true*
...
<!-- VERIFICATION REPORT
Node-Specific Properties: 16 documented (TransformSpace, CustomTransform, VoxelSizeMode, VoxelSize, MaxCellSize, MinVoxelsPerCell, Balance, AxisBias, Weight, WeightInfluence, WeightMode, PriorityThreshold, PreferredMinVolume, PreferredMaxVolume, VolumePreferenceWeight, bUseHeuristicMergeGating, MergeScoreThreshold)
