---
icon: route
description: 'Pathfinding : Edges - Finds paths through edge clusters from seeds to goals'
---
...
|----------|-------------|
| **Picking Method** | Edge (closest edge) or Node (closest vertex) |
| **Max Distance** | Maximum search distance (-1 for unlimited) |
...
| Property | Description |
|----------|-------------|
| **Add Index Tag** | Add seed index as a tag |
...
| Property | Description |
|----------|-------------|
| **Remove Small Paths** | Exclude paths below minimum point count |
| **Min Point Count** | Minimum points required (default: 3) |
| **Remove Large Paths** | Exclude paths above maximum point count |
| **Max Point Count** | Maximum points allowed (default: 500) |
...
Node-Specific Properties: 14+ documented
  - GoalPicker, bAddSeedToPath, bAddGoalToPath, PathComposition
  - SeedPicking, GoalPicking, SearchAlgorithm, HeuristicScoreMode
  - SeedAttributesToPathTags, SeedForwarding, GoalAttributesToPathTags, GoalForwarding
  - Statistics, PathOutputDetails, bUseOctreeSearch, bGreedyQueries
Inherited Properties: Referenced to UPCGExClustersProcessorSettings
Inputs: Vtx (Points), Edges (Points), Seeds (Points), Goals (Points), Heuristics (Params)
Outputs: Paths (Points)
Nested Types: EPCGExPathComposition, FPCGExNodeSelectionDetails, FPCGExAttributeToTagDetails, FPCGExForwardDetails, FPCGExPathStatistics, FPCGExPathOutputDetails
-->
