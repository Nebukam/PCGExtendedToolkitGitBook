---
icon: warehouse
description: 'Staging : Distribute - Distribute PCGEx Asset Collection entries to points'
---
...
<details>
<summary><strong>Asset Collection</strong> <code>TSoftObjectPtr<UPCGExAssetCollection></code></summary>
...
📋 *Visible when Collection Source = Asset*
...
📋 *Visible when Collection Source = Attribute Set*
...
📋 *Visible when Collection Source = Path Attribute*
...
📋 *Visible when Output Mode = Point Attributes*
...
Controls how assets are positioned (justified) within point bounds after scaling. Each axis can be independently justified (center, min, max, or custom blend). Supports:
- Per-axis justification enable/disable
- Justification mode per axis
- Optional shared custom justification from attributes

Default: Centered justification on all axes
...
📋 *Visible when Do Filter Entry Type is enabled*
...
📋 *Visible when Output Mode ≠ Collection Map*
...
📦 **Module**: `PCGExCollections` · 📄 [Source](https://github.com/Nebukam/PCGExtendedToolkit/blob/main/Source/PCGExCollections/Public/Elements/PCGExAssetStaging.h)
...
Inherited Properties: Referenced to point filtering base class
