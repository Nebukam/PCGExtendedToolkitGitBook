---
icon: circle-dashed
description: 'BlendOp : Blend All - Creates bulk blend operations from monolithic blending settings.'
---
...
* **Bulk vs Individual**: This node replaces multiple individual BlendOp sub-nodes when you want the same (or similar) treatment across many attributes. Use individual BlendOps when you need per-attribute operand control, custom weights, or transient chaining.
* **Supersede Behavior**: When both monolithic and individual BlendOps target the same attribute, individual BlendOps take priority and the monolithic operation for that attribute is skipped.
...
<!-- VERIFICATION REPORT
Node-Specific Properties: 2 documented (Priority, BlendingDetails)
Inherited Properties: Referenced to UPCGExFactoryProviderSettings
Inputs: Labels
Outputs: Blending (Blend Op Factory)
Nested Types: FPCGExBlendingDetails (referenced to common-settings page)
-->
